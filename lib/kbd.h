 ...
 *
 *       The keyboard controller has four 8-bit registers involved in
 *       communication with the CPU: its input buffer, that can be written by
 *       the CPU by writing port 0x60 or port 0x64; its output buffer, that can
 *       be read by the CPU by reading from port 0x60; and the status register,
 *       that can be read by the CPU by reading from port 0x60.
 *
 *       If the CPU writes to port 0x64, the byte is interpreted as a command
 *       byte. If the CPU writes to port 0x60, the byte is interpreted as a data
 *       byte.
 *

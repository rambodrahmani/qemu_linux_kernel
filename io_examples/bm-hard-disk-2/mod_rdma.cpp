...
/**
 * Primitive 252 Assembly implementation.
 */
extern "C" void a_go_rdma();

/**
 * Primitive 52 Assembly implementation.
 */
...
void ini()
{
    // find and initialize IDE bus master
    init_bm();

    // initialize IDT entry 252 with a_go_rdma address
    gate_init(252, &a_go_rdma);

    // initialize IDT entry 52 with a_driver_rdma address
    gate_init(52, &a_driver_rdma);

    // set interrupt 52 for APIC pin 14
    apic_set_VECT(14, 52);

    // enable APIN pin 14 interrupt requests
    apic_set_MIRQ(14, false);
}

/**
 * C++ body implementation for primitive a_go_rdma.
 */
extern "C" void c_go_rdma(natl indse, natb nn, natb vv[])
{
    natb work;
    natq ww;
    ...
    // bit n. 2 set to 0 (PCI read transfer)
    work |= 0x08;

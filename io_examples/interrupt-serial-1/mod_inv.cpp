...
// uscita dati a interruzione di programma
...
const natl sincr_s = 4;			// si usa il semaforo numero 13
natl cont;
char* punt;
...
extern "C" void a_invia_serial();
extern "C" void a_driverout_serial();
void ini()
{	ini_ip_COM1();
	gate_init(244, &a_invia_serial);
	gate_init(44, &a_driverout_serial);
	apic_set_VECT(4, 44); 
	apic_set_MIRQ(4, false);		// abilit. piedino n. 4 dello IO APIC 
}

extern "C" void c_invia_serial(natl nn, char vv[])
{	cont = nn; 
	punt = vv;
	outputb(0x02, iIER);			// abilit. richieste di interruzione uscita
	sem_wait(sincr_s);
}
...
{	natb a;
	...
	a = *punt; 				// trasferimento dalla memoria
	outputb(a, iRBR);
	punt++;
}
//****************************************************************************************************

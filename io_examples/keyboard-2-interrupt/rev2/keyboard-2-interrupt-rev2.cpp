 ...
 *       Revision 2.
 *       The aim of this revision is to introduce a new source of interrupt
 *       requests to point out how the APIC manages different priorities and how
 *       does this mangement affect the flow control.
 *       As a secondary source for interrupt requests we will be using counter 0
 *       which is connected to pin number 2 of the APIC. The counter interrupt
 *       requests will be used to generate a new spinner rotating on the right
 *       of the one executed by the main program.
 *
 *       Compile using:
 *          ./compile
 *
 ...
 *
 *       Output:
 *          When we execute this implementation. We will have the two spinner
 *          spinning on top of the video output. When a key is pressed and
 *          released, we can notice both spinners will stop while the make code
 *          is being printed. After that, the timer driven spinner will execute
 *          a single step, both of the spinner will hang again and the break
 *          code will be printed. [3]
 *

...
/**
 * CE devices loaded at boot.
 */
des_ce array_ce[MAX_CE];

/**
 * Number of effective CE devices loaded at boot.
 */
natl next_ce;
// EXTENSION 2016-06-15
...
extern "C" void c_cedmaread(natl id, char *buf, natl quanti)
{
    ...
    // get the number of bytes available in the page containing the buffer
    natw rem = 4096 - ((natq)f & 0xfff);

    // if there more bytes available in the page than the one to be transferred
    if (rem > quanti)
    {
        // assign number of bytes to be transferred to the remaning bytes value
        rem = quanti;
    }
    ...
    // update destination buffer pointer address: value after transfer
    c->buf = buf + rem;

    // update number of bytes to be transferred: value after transfer
    c->quanti = quanti - rem;

    // write destination buffer physical address
    outputl((natq)f, c->iBMPTR);

    // write updated number of bytes to be trasferred
    outputl(rem, c->iBMLEN);

    // write to the command register: start transfer
    outputl(1, c->iCMD);

    // wait for the the sync semaphore: set by estern_ce
    sem_wait(c->sync);
 ...
 *
 * @param  id  external CE device id.
 */
extern "C" void estern_ce(int id)
{
    ...
    for (;;)
    {
        ...
        if (c->quanti > 0)
        {
            ...
            if (rem > 4096)
            {
                // if it is, set next transfer bytes to 4 Kib
                rem = 4096;
            }
            ...
            // update destination buffer address pointer: value after transfer
            c->buf += rem;

            // update number of byte to be transferred: value after transfer
            c->quanti -= rem;

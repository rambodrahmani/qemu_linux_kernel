...
def show_lines_asm(rip, indent=0):
    lines = gdb.execute("disassemble {}".format(rip), False, True)
    found = False
    for l in lines.splitlines():
        if not found:
            w = l.split()
            try:
                if len(w) > 0 and int(w[0], 0) == rip:
                    gdb.write(" " * indent + colorize('col_curline', l.expandtabs()) + "\n")
                    found = True
                    continue
            except:
...
def print_context():
    ...
    gdb.write("-" * 80 + "\n")
    gdb.write(colorize('col_var', "b_info:\n"))
    gdb.write(colorize('col_var', "  busy: ") + str(gdb.parse_and_eval("b_info.busy")) + "\n")
    for w in ("waiting", "intercepted", "to_wakeup"):
        gdb.write(colorize('col_var', "  " + w + ": ") + show_list(gdb.parse_and_eval("b_info."+w), 'puntatore') + "\n")
    gdb.write(colorize('col_var', "  rip: ") + str(gdb.parse_and_eval("b_info.rip")) + "\n")
    gdb.write(colorize('col_var', "  orig: ") + str(gdb.parse_and_eval("b_info.orig")) + "\n")

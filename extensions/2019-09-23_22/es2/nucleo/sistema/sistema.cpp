...
// ( ESAME 2019-09-23
	// se il processo e' slave lo disassociamo dal master
	if (pdes_proc->bp_master) {
		void deduplica(natl id, vaddr v);

		// deduplica non chiamata dal master non ripristinera'
		// correttamente la tabella di livello 4 del processo
		// corrente, ma non importa perche' va comunque distrutta
		deduplica(esecuzione->id, pdes_proc->bp_addr);

		if (pdes_proc->bp_waiting) {
			inserimento_lista(pronti, pdes_proc->bp_waiting);
		}

		des_proc *master = pdes_proc->bp_master;

		master->bp_slave = 0;
		master->bp_slave_id = 0xFFFFFFFF;
	}
//   ESAME 2019-09-23 )
...
// spazio di indirizzamento del processo di identificatore id
// (il processo deve esistere)
faddr trasforma(natl id, vaddr ind_virt)
{
	natq d;
...
{
	return trasforma(esecuzione->id, ind_virt);
}


extern "C" int c_getnframe()
{
	int count = 0;
	for (natq i = 0; i < N_DF; i++)
		if (vdf[i].livello == -1)
...
extern "C" void cmain()
{
	...
	flog(LOG_INFO, "Nucleo di Calcolatori Elettronici, v5.12.6");
	init_gdt();
...
extern "C" void c_bpwait()
{
	...
	if (!self->bp_slave) {
		flog(LOG_WARN, "bpwait errata");
		c_abort_p();
		return;
	}

	// se lo slave non e' bloccato sulla coda del master
	// inseriamo il master nella coda della slave
	if (!self->bp_waiting) {
...
// invocata ogni volta che un processo cerca di eseguire 0xCC (int3)
extern "C" void c_breakpoint_exception(int tipo, natq errore, vaddr *p_rip)
{
	des_proc *self = des_p(esecuzione->id);

	// se il processo non ha un master, va abortito
	if (!self->bp_master) {
		gestore_eccezioni(tipo, errore, *p_rip);
		return;
	}

	// riportiamo indietro di uno il rip salvato in pila
	natq* rip = reinterpret_cast<natq*>(p_rip);
	(*rip)--;

	// inseriamo il processo nella coda del master e,
	// se questo era bloccato in attesa, lo risvegliamo
...
bool duplica(natl id, vaddr v)
{
	...
	for (int i = 3; i >= 0; i--) {
		...
		// riempiamo i campi del descrittore di frame della copia
...
extern "C" void c_bpattach(natl id, vaddr rip)
{
	...
	if (self->bp_slave) {
		flog(LOG_WARN, "bpattach duplicata");
		c_abort_p();
		return;
	}
	...
	// master di se stesso
	if (dest == self) {
		flog(LOG_WARN, "bpattach ricorsiva");
		c_abort_p();
		return;
	}

	// il rip deve appartenere alla zona utente/condivisa
	...
	if (self->bp_master || !dest || dest->bp_master || dest->bp_slave) {
		return;
	}
	...
	// duplichiamo la traduzione della pagina che contiene rip,
	// in modo che il processo id abbia una sua copia privata
	if (!duplica(id, rip))
		return;
	
	// ora dobbiamo scrivere 0xCC nel byte di indirizzo rip nella memoria
	// dello slave; non possiamo pero' usare direttamente rip, perche' la
	// MMU lo tradurrebbe usando l'albero del processo corrente (che e' il
	// master) e non quello dello slave; allora lo trasformiamo in fisico
	// via software poi vi accediamo tramite la finestra FM
	faddr frip = trasforma(id, rip);
	natb *bytes = reinterpret_cast<natb*>(frip);
	dest->bp_addr = rip;
	dest->bp_orig = *bytes;
	*bytes = 0xCC;
}

// disfa quanto fatto fa duplica(id, v)
void deduplica(natl id, vaddr v)
{
	// dobbiamo procedere dalla pagina verso le tabelle di livello 1, 2 e
	// 3 in quest'ordine, altrimenti cancelliamo le tabelle prima di usarle
	for (int i = 0; i <= 3; i++) {
		tab_entry e = get_des(id, i + 1, v);
	...
	// per ripristinare l'entrata nella tabella di livello 4 sfruttiamo il
	// fatto che tutti i processi che non sono slave puntano alla stessa
	// tabella di livello 3.  Poiche' un master non puo' essere anche slave,
	// usiamo la tabella del master.
...
extern "C" void c_bpdetach()
{
	...
	deduplica(self->bp_slave_id, dest->bp_addr);
...
// bit del registro dei flag che abilita il single-stap trap
const natq TRAP_FLAG = 1UL << 8;

void enable_single_step(natq& flags)
{
	flags |= TRAP_FLAG;
}

void disable_single_step(natq& flags)
{
	flags &= ~TRAP_FLAG;
}

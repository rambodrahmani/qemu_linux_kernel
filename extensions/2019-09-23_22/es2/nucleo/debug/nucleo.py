...
des_proc_std_fields = [ None, 'cr3', 'contesto', 'cpl', 'bp_master', 'bp_slave' ]
...
def show_lines_asm(rip, indent=0):
    lines = gdb.execute("disassemble {}-1, +20".format(rip), False, True)
    found = False
    for l in lines.splitlines():
        if not found:
            w = l.split()
            try:
                if len(w) > 0 and int(w[0][:-1], 0) == rip:
                    gdb.write(" " * indent + colorize('col_curline', l.expandtabs()) + "\n")
                    found = True
                    continue
            except:
...
def process_dump(proc, indent=0, verbosity=3):
    ...
    if (verbosity > 2):
        ...
        show_lines_asm(v2p(proc, rip), indent)
    if len(toshow) > 0:
        if verbosity > 2:
...
class ProcessBp(gdb.Command):
    """list processes whose bp_waiting field is non-null"""

    def __init__(self):
        super(ProcessBp, self).__init__("process bp", gdb.COMMAND_DATA)

    def invoke(self, arg, from_tty):
        for pid, proc in process_list():
            if proc['bp_waiting'] == gdb.Value(0):
                continue
            gdb.write("process {}: bp_waiting {}\n".format(pid, proc['bp_waiting']))

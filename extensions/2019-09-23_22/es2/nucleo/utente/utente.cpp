...
int sync;
int sync2;
#line 128 "utente/utente.cpp"

int main()
{
	bad1 = activate_p(bad, 1, 40, LIV_UTENTE);
	bad2 = activate_p(bad, 2, 16, LIV_UTENTE);
	usr1 = activate_p(usr, 2, 10, LIV_UTENTE);
	usr2 = activate_p(usr, 3, 23, LIV_UTENTE);
	usr3 = activate_p(usr, 1, 22, LIV_UTENTE);
	dbg1 = activate_p(debugger, usr1, 20, LIV_UTENTE);
	dbg2 = activate_p(debugger, usr3, 6, LIV_UTENTE);

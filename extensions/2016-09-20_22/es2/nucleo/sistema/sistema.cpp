...
struct broadcast
{
    ...
    // number of registered processes waiting for a broadcast message
    int nlisten;

    // last broadcast message
    natl msg;

    // processes waiting for the next broadcast message queue
    proc_elem *listeners;

    // broadcaster wait queue for all registered processes
...
/**
 * Implementation for the void reg() implementation.
 */
extern "C" void c_reg()
{
    /**
...
extern "C" void cmain()
{
	...
	flog(LOG_INFO, "Nucleo di Calcolatori Elettronici, v5.12.5");
	init_gdt();
